    ...
    <div class="articlepost">
       ...
       <p>Both the Server <i>and</i> the Client have the pre-master secret. They want to obtain the <i>master</i>  secret, which will be a <i>pseudo</i>  random number. But they should ensure that <i>both of them  will arrive to the same pseudo random number</i>. For that they will use a function that will get as input some of the data they already share between them...</p>
        ...
        <div class='step' data-step='12'>
            <div class="explain">
            <p>The reason why several session keys instead of one is because in reality there are more than one type of communication during a single session. Encrypting each type of communication with a different key adds a higher level of security.</p>
            </div>
        </div>
        <div class='step' data-step='13'>
            <div class="explain">
            <p>The expected outcome of the symmetric encryption is not different to the one previously described for assymetric encryption: using a cipher function with a key to produce a ciphertext from a plaintext. In order to test that everything is correct, the Client encrypt a convined message ("finished") and sends it to the server...</p>
            </div> 
        </div>
        ...
        <div class='step' data-step='15'>
            <div class="explain">
            <p>If after receiving the ciphertext and decrypting it with the corresponding copy of the session key, </p><p>if the message is as expected, the Server responds to the Client with a similar message... </p>
            </div> 
        </div> 
        ...
        <div class='step' data-step='15'>
            <div class="explain">
            <p>If the Client gets the message, then <strong>the handshake is finished</strong>. </p>
            </div> 
        </div>            
    </div>
    <br>
